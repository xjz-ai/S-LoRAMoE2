...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    """
    testpath = '/home/xjz/proj/S-LoRAMoE2/test_expert_num.json'
    name_experts = []
    with open(testpath, 'r', encoding='utf-8') as file:
        # 使用 json.load() 方法将文件内容解析为 Python 字典
        name_experts = json.load(file)
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                    ...
                    print("linear")
                    if key in self.name_experts:
                        lora_num = self.name_experts[key]
                        print(key)
                        print(lora_num)
                    else:
                        lora_num = 0
                        print(key)
                        print('none')
                    new_module = Linear(target.in_features, target.out_features,lora_num = lora_num, bias=bias, **kwargs)
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            if self.lora_num !=0:
                ...
                setattr(self, f"lora_B", nn.Linear(self.lora_num*r, out_features, bias=False))
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                ...
                if self.lora_num>1:
                    ...
                    route_weight = route_weight.repeat_interleave(self.r, dim=2)
                # 原创写法，我们后续的工作会解释这一写法，可以加速moe的前向传播，代替for循环,也可以自行改写为以下形式
                # for i in range(self.lora_num):
                #     result = result + torch.unsqueeze(route_weight[:,:,i], -1) * getattr(self, f"lora_B{i}")(getattr(self, f"lora_A")(self.lora_dropout(x))) * 8.0
                temp =((getattr(self, f"lora_A")(self.lora_dropout(x))))
                temp = temp.repeat(1, 1, self.lora_num)
                if self.lora_num>1:
                    temp = temp*route_weight
                temp = getattr(self, f"lora_B")(temp) * self.scaling
                result = result+temp
        else:   
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
